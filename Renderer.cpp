...
Bunny_Animation* Renderer::m_bunny_animation = new Bunny_Animation();

bool Renderer::keys[1024];
bool bool_curve, bool_bunny, bool_bunny_ease, bunny_rot;
float tmax;
...
void Renderer::nanogui_init(GLFWwindow* window)
{
	...
	static auto curve_widget = gui_1->addVariable("Catmull-Rom curve on/off", bool_curve);

	gui_1->addGroup("Bunny animation");
	static auto bunny_widget = gui_1->addVariable("Bunny is moving", bool_bunny);
	static auto t1_widget = gui_1->addVariable("t1", m_bunny_animation->t1);
	static auto t2_widget = gui_1->addVariable("t2", m_bunny_animation->t2);
	static auto total_widget = gui_1->addVariable("total moving time", m_bunny_animation->tmax);
	static auto bunny_rot_widget = gui_1->addVariable("Bunny is rotating", bunny_rot);

	gui_1->addButton("Reset movement", []() {
		m_bunny_animation->reset();
		bunny_widget->setChecked(false);
		bool_bunny = 0;
	});
...
void Renderer::init()
{
	...
	m_lightings->init();
	m_curve->init();
	m_bunny_animation->init(m_curve);
...
void Renderer::load_models()
{
	...
	Object bunny_object("./objs/bunny.obj");
	bunny_object.obj_color = glm::vec4(1.0, 1.0, 1.0, 1.0);
	bunny_object.obj_name = "bunny";

	Object curve_object(m_curve->get_points);
	...
	obj_list.push_back(bunny_object);
	...
	for (size_t i = 0; i < obj_list.size(); i++)
	{
		if (obj_list[i].obj_name == "cube")
		{
			...
			for (unsigned int j = 0; j < m_curve->control_points_pos.size(); j++) {
				...
				//rotate
				glm::mat4 curve_quat_mat = m_curve->quat_to_rotation_mat(m_curve->control_points_quaternion[j]);
				for (int a = 0; a < 4; a++)
				{
					for (int b = 0; b < 4; b++)
					{
						...
						for (int c = 0; c < 4; c++)
						{
							result_mat[a][b]+= curve_quat_mat[a][c] * cur_obj_model_mat[c][b];
						}
					}
				}
				for (int a = 0; a < 4; a++)
				{
					for (int b = 0; b < 4; b++)
					{
						cur_obj_model_mat[a][b] = result_mat[a][b];
					}
				}
		...
		if (obj_list[i].obj_name == "bunny")
		{
			glm::mat4 cur_bunny_model_mat = glm::mat4(1.0f);
			if (bool_bunny == 1 && bool_curve == 1 && bunny_rot==1)
			{
			m_bunny_animation->update(delta_time, bunny_rot);
			}
			else if(bool_bunny == 1 && bool_curve == 1 && bunny_rot == 0)
			{
				m_bunny_animation->update(delta_time, bunny_rot);
			}
			cur_bunny_model_mat = m_bunny_animation->get_model_mat();
			glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1, GL_FALSE, glm::value_ptr(cur_bunny_model_mat));
			draw_object(shader, obj_list[i]);
			// Draw axis for bunny
			draw_axis(shader,cur_bunny_model_mat);
		}
